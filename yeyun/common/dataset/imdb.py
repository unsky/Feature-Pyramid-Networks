...
class IMDB(object):
    ...
    def append_flipped_images(self, roidb):
        ...
        for i in range(self.num_images):
            ...
            oldx1 = boxes[:, 0].copy()
            oldx2 = boxes[:, 2].copy()
            boxes[:, 0] = float(roi_rec['width']) - oldx2 - 1.0
            boxes[:, 2] = float(roi_rec['width']) - oldx1 - 1.0
            assert (boxes[:, 2] >= boxes[:, 0]).all()
            
            entry = dict()
            if roi_rec.has_key('extra_rois'):
                extra_rois = roi_rec['extra_rois'].copy()
                oldx1 = extra_rois[:, 0].copy()
                oldx2 = extra_rois[:, 2].copy()
                extra_rois[:, 0] = float(roi_rec['width']) - oldx2 - 1.0
                extra_rois[:, 2] = float(roi_rec['width']) - oldx1 - 1.0
                assert (extra_rois[:, 2] >= extra_rois[:, 0]).all()
                entry['extra_rois'] = extra_rois                
            if roi_rec.has_key('interested_regions'):
                interested_regions = roi_rec['interested_regions'].copy()
                oldx1 = interested_regions[:, 0].copy()
                oldx2 = interested_regions[:, 2].copy()
                interested_regions[:, 0] = float(roi_rec['width']) - oldx2 - 1.0
                interested_regions[:, 2] = float(roi_rec['width']) - oldx1 - 1.0
                assert (interested_regions[:, 2] >= interested_regions[:, 0]).all()
                entry['interested_regions'] = interested_regions
            if roi_rec.has_key('ignore_regions'):
                ignore_regions = roi_rec['ignore_regions'].copy()
                oldx1 = ignore_regions[:, 0].copy()
                oldx2 = ignore_regions[:, 2].copy()
                ignore_regions[:, 0] = float(roi_rec['width']) - oldx2 - 1.0
                ignore_regions[:, 2] = float(roi_rec['width']) - oldx1 - 1.0
                assert (ignore_regions[:, 2] >= ignore_regions[:, 0]).all()
                entry['ignore_regions'] = ignore_regions
                
            if roi_rec.has_key('gt_classes'):
                entry['gt_classes'] = roi_rec['gt_classes']
                
            if roi_rec.has_key('attributes'):
                entry['attributes'] = roi_rec['attributes']
            
            entry.update({'image': roi_rec['image'],
