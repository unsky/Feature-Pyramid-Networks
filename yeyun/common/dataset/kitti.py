...
class kitti(IMDB):
    def __init__(self, image_set,root_path, devkit_path=None,category='all'):
        super(kitti, self).__init__('kitti__', image_set, root_path, devkit_path)  # set self.name 
        self.image_set = image_set
        self.num_classes = 4
        self.devkit_path = self._get_default_path() if devkit_path is None else devkit_path
       
        self.data_path = os.path.join(self.devkit_path, image_set+'ing/image_2')
        ...
        self.comp_id = 'comp4'
        assert os.path.exists(self.devkit_path), 'Kitti path does not exist: {}'.format(self.devkit_path)
    ...
    def _load_image_set_index(self):
        ...
        if self.image_set  == 'test':
            image_set_file = os.path.join(
                self.devkit_path, 'ImageSets','train.txt')
    ...
    def rpn_roidb(self):
        if  self.image_set != 'test':
            ...
            roidb = self.load_rpn_roidb(None)
    ...
    def _load_kitti_annotation(self, index):
        ...
        """
        if self.image_set=='test':
            imagename = os.path.join(self.devkit_path,'testing/image_2',index+'.png')
        ...
        filename = os.path.join(self.devkit_path, 'training/label_2', index + '.txt')
        imagename = os.path.join(self.devkit_path,'training/image_2',index+'.png')
        img = cv2.imread(imagename)
        width = img.shape[0]
        height = img.shape[1]
        f = open(filename)
        ...
        for l in lines:
            str_cls = l.split()
            if str(str_cls[0]) in self.classes or str(str_cls[0])=='Van':
                 num_objs = num_objs + 1
        num_objs = num_objs
#        print 'num_objs',num_objs
        ...
        for line in lines:
            ...
            if str(data[0]) =='Van':
                data[0] = 'Car'
            if str(data[0]) not in self.classes:
                 continue
            x1 = int(float(data[4]))
            y1 = int(float(data[5]))
            x2 = int(float(data[6]))
            y2 = int(float(data[7]))
            cls = self.class_to_ind[data[0]]
 ...
 #       a
    ...
    def _do_python_eval(self, output_dir = 'output'):
        annopath = os.path.join(self.devkit_path, 'training/label_2', '{:s}.txt') 
...
#                os.remove(filename)   
    ...
    def _get_voc_results_file_template(self):
        ...
        filename = self.comp_id + '_det_' + self.image_set + '_{:s}.txt'
        path = os.path.join(
            'data',
            'results',
            'Main',
