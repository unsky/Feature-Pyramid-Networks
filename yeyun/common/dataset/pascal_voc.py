...
class PascalVOC(IMDB):
    def __init__(self, image_set, root_path, devkit_path, category='all'):
        """
    ...
    def load_pascal_annotation(self, index):
        ...
        for ix, obj in enumerate(objs):
            ...
            if not class_to_index.has_key(obj.find('name').text.lower().strip()):
                continue
        ...
        for ix in range(num_objs):
            boxes[ix, :] = boxes_list[ix]
            gt_classes[ix] = gt_classes_list[ix]
    ...
    def evaluate_detections(self, detections):
        ...
        print 'evaluate detection results with voc tools'
    ...
    def draw_roc_curve(self, cls, rec, prec, ap):
        """
        draw roc curve 
        :return: None
        """
        plt.plot(rec, prec, color='r', label='{},{:.2f}'.format(cls, ap))
        plt.legend(loc="best")
        plt.xlabel('recall')
        ...
        plt.yticks(np.arange(0.,1,0.05), fontsize = 8)
        plt.grid(True)
        plt.savefig(self.cache_path+'/roc_'+cls+'.jpg')
    ...
    def do_python_eval(self):
        ...
        result_file = os.path.join(self.cache_path, self.name + 'eval_result.txt')
        result_cont = ''
        recs = []
        precs = []
        aps = []
        ...
        with open(result_file, 'wt') as f:
            f.write(result_cont)
        os.system('mailx -s "gpu029 voc eval result" 423876867@qq.com < '+result_file)
...
'''if __name__ == "__main__":
    image_set_name = '2007_debug2'
    root_path = 'debug'
    dataset_path = 'data/VOCdevkit'
    imdb = PascalVOC(image_set_name, root_path, dataset_path)
    roidb = imdb.gt_roidb()'''
