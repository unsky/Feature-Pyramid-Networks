...
import time
def get_rpn_batch(roidb, scales, image_mean, stride=0):
    """
    ...
    imgs, roidb = get_image(roidb, scales, image_mean, stride=stride)
    im_array = imgs[0]
...
def assign_anchor(feat_shape, gt_boxes, im_info, config, feat_stride=16,
...
def fpn_assign_anchor(feat_shape, gt_boxes, im_info, config, feat_stride=[4,8,16,32,64],
                  scales=(8), ratios=(0.5, 1, 2), allowed_border=1):
    """
    assign ground truth boxes to anchor positions
...
#         print 'rpn: max max_overlaps', np.max(max_overlaps)
    ...
    if len(feat_shape) == 3:
        label = {'n': labels1, 'label2': labels2, 'label3': labels3,
            'bbox_target1': bbox_targets1, 'bbox_target2': bbox_targets2, 'bbox_target3': bbox_targets3,
            'bbox_weight1': bbox_weights1, 'bbox_weight2': bbox_weights2, 'bbox_weight3': bbox_weights3}
