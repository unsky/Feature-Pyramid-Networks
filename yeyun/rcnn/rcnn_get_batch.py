...
from bbox_regression import expand_bbox_regression_targets
...
def get_rcnn_batch(roidb, config):
    """
...
def destrib_rois(rois, k0=4):
    """
    destribute different size of rois to different depth layers of net.
    :param rois: [300, 5]
    :param k0: project 224*224 roi to layer:k0
    :return: layer_indexs: [n]
    """
    w = rois[:,3]-rois[:,1]
    h = rois[:,4]-rois[:,2]
    s = w * h
...
def sample_rois(rois, fg_rois_per_image, rois_per_image, num_classes,
                labels=None, overlaps=None, bbox_targets=None, gt_boxes=None, sample_type=None):
    """
